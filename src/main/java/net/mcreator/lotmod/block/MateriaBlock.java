...
import net.mcreator.lotmod.procedures.MateriaMobplayerCollidesBlockProcedure;
...
public class MateriaBlock extends LotmodModElements.ModElement {
	@ObjectHolder("lotmod:materia")
	public static final FlowingFluidBlock block = null;
	@ObjectHolder("lotmod:materia_bucket")
	...
	public void initElements() {
		fluidproperties = new ForgeFlowingFluid.Properties(() -> still, () -> flowing,
				FluidAttributes.builder(new ResourceLocation("lotmod:blocks/cvvnbfg"), new ResourceLocation("lotmod:blocks/cvvnbfg")).luminosity(12)
						.density(1000).viscosity(1000).gaseous()).bucket(() -> bucket).block(() -> block);
		still = (FlowingFluid) new ForgeFlowingFluid.Source(fluidproperties).setRegistryName("materia");
		flowing = (FlowingFluid) new ForgeFlowingFluid.Flowing(fluidproperties).setRegistryName("materia_flowing");
		elements.blocks.add(() -> new FlowingFluidBlock(still, Block.Properties.create(Material.WATER)) {
			...
			public void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity) {
				...
				int z = pos.getZ();
				{
					...
					MateriaMobplayerCollidesBlockProcedure.executeProcedure($_dependencies);
				}
			}
		}.setRegistryName("materia"));
		elements.items.add(() -> new BucketItem(still, new Item.Properties().containerItem(Items.BUCKET).maxStackSize(1).group(LotmodcrItemGroup.tab))
				.setRegistryName("materia_bucket"));
	}
